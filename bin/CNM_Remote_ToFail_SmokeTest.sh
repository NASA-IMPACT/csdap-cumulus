...
# CNM_Remote_ToFail_SmokeTest.sh

# This test is designed to be used to make sure that the remote topics can receive a failed signal.
...
echo ""
echo "Running CNM_Remote_ToFail_SmokeTest.sh -- Note, this test should work in all environments."
echo "The purpose of this test is to send a small message to the remote submission topic, verify that the state machine fails, then verify that a 'fail' response topic message was sent."
...
PUBLISH_COMMAND="aws sns publish --topic-arn \"${TOPIC_ARN}\" --message \"${JSON_MESSAGE}\" --subject \"Cumulus CNM_Remote_ToFail_SmokeTest\""
aws sns publish --topic-arn "${TOPIC_ARN}" --message "${JSON_MESSAGE}" --subject "Cumulus CNM_Remote_ToFail_SmokeTest" # : $(date -u +%Y-%m-%dT%H:%M:%SZ)"
...
echo "In Sandbox and UAT, check email for both remote submission and response topics (note, you have to subscribe to the topics with email to see this)"
echo "In PROD, there should be a lambda hooked up to the remote response topic which should simply print data to it's cloud watch log group.  Look for that log output as a verification."
echo ""
