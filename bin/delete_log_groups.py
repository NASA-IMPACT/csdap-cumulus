# delete_log_groups.py
# ORCA Upgrade from v10.1.0 to v10.1.2
# https://github.com/nasa/cumulus-orca/blob/v10.1.2/bin/delete_log_groups.py
# Ref: https://github.com/nasa/cumulus-orca/releases/tag/v10.1.1

import subprocess
import json

# Gets user input for the prefix of lambdas to delete
prefix = input("Enter Prefix: ")                # Example: uat or prod (or sandbox prefix)
aws_profile = input("Enter AWS Profile: ")      # The local AWS profile that has permissions on the environment you want to modify.
...
get_functions = f"aws lambda list-functions --query 'Functions[] | [?contains(FunctionName, `{prefix}`) == `true`]' --profile {aws_profile}"
...
for sub in convert:
    ...
    get_tags = f"aws lambda list-tags --resource {lambda_output} --profile {aws_profile}"
            ...
            for attr in tag_convert[tag]:
                if attr == 'application':
                    if tag_convert[tag]['application'] == 'ORCA':
                        ...
                        delete_log_group = f"aws logs delete-log-group --log-group-name /aws/lambda/{del_log_group} --profile {aws_profile}"
