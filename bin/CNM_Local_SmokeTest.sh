...
PUBLISH_COMMAND="aws sns publish --topic-arn \"${TOPIC_ARN}\" --message \"${JSON_MESSAGE}\" --subject \"Cumulus CNM_Local_SmokeTest\""
aws sns publish --topic-arn "${TOPIC_ARN}" --message "${JSON_MESSAGE}" --subject "Cumulus CNM_Local_SmokeTest" # : $(date -u +%Y-%m-%dT%H:%M:%SZ)"
